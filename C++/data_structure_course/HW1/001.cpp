...
int other(long long att,double ats){
	fstream myfile;
	cout << fre << endl;
	cout<<"\nInsertionsort time-->"<<att<<"ns, "<<ats<<"s\n";
	myfile.open("count.txt",ios::app);      //改app
...
int main(){
    ...
    // ----------------------------------------------------------------------------------------------
    	...
    	att=chrono::duration_cast<chrono::nanoseconds>((end-start)/smfre).count();    //秒 seconds
		ats=att*0.000000001;
		other(att,ats);
    //-------------------------------------------------------------------------------------------------
        randd(vs,vi,vlo,vd,vf,as,ai,alo,ad,af,aas,aai,aalo,aad,aaf);
